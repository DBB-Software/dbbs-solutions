    ...
    async def get_all(
        ...
        if all([filter_field, filter_operator, filter_value]):
            if filter_operator in map_filter_db:
                field = getattr(TypeModel, filter_field, None)
                if field:
                    query = query.where(
        ...
        if sort_by:
            field = getattr(TypeModel, sort_by, None)
            if field:
        ...
        entities: List[TypeModel] = list(result.scalars().unique())

        return to_pagination_mapper(entities, offset, count, remaining)


async def get_type_service(db_session: Annotated[AsyncSession, Depends(get_db_session)]) -> TypeService:
